/* LAX OSINT — الترجمات (عربي/إنجليزي) والتبديل RTL/LTR */
(function () {
  "use strict";

  const DICT = {
    ar: {
      tagline: "اكتشف بصمتك الرقمية",
      search: "بحث",
      searching: "جارٍ البحث…",
      resultsFor: "نتائج البحث عن",
      username: "يوزرنيم",
      email: "إيميل",
      phone: "رقم هاتف",
      placeholder_username: "مثال: darkhacker",
      placeholder_email: "مثال: name@example.com",
      placeholder_phone: "مثال: +1 555 123 4567",
      help_username: "فحص اليوزر نيم في أكثر من 3000 موقع",
      help_email: "فحص الإيميل في أكثر من 120 منصة + صورة Gravatar",
      help_phone: "استخراج معلومات تقنية عن رقم الهاتف",
      quota: "بحوثك اليوم",
      pro: "برو",
      login: "تسجيل الدخول",
      register: "إنشاء حساب",
      logout: "خروج",
      activatePro: "تفعيل كود برو",
      welcome: "مرحبًا",
      loginTitle: "تسجيل الدخول",
      registerTitle: "إنشاء حساب جديد",
      emailLabel: "البريد الإلكتروني",
      passwordLabel: "كلمة المرور",
      noAccount: "ليس لديك حساب؟ سجّل الآن",
      hasAccount: "لديك حساب؟ تسجيل الدخول",
      proTitle: "تفعيل كود برو",
      proSub: "أدخل كود التفعيل للحصول على بحث غير محدود لمدة 30 يومًا",
      codeLabel: "الكود (XXXX-XXXX-XXXXX-XXX)",
      codePlaceholder: "مثال: HSDFN-DSLF-84357-FFJ",
      activate: "تفعيل",
      cancel: "إلغاء",
      status_registered: "مؤكد",
      status_not_registered: "غير مسجّل",
      status_rate_limited: "حد الاستعلام",
      status_warning: "تنبيه",
      status_error: "خطأ",
      foundResults: "تم العثور على",
      resultsLabel: "نتائج مؤكدة",
      noResults: "لا توجد نتائج",
      done: "تم — عدد النتائج",
      seconds: "ثانية",
      searchFailed: "تعذر تنفيذ البحث",
      limitReached: "وصلت لحد البحوث المجانية لهذا اليوم — فعّل كود Pro للمتابعة",
      loginRequired: "يجب تسجيل الدخول أولًا",
      welcomeBack: "تم تسجيل الدخول بنجاح",
      registered: "تم إنشاء الحساب بنجاح",
      proActivated: "تم تفعيل البرو بنجاح — بحث غير محدود حتى",
      weakPassword: "كلمة المرور قصيرة جدًا",
      exists: "هذا الإيميل مسجل بالفعل",
      badPassword: "كلمة المرور غير صحيحة",
      notFound: "لا يوجد حساب بهذا الإيميل",
      badFormat: "صيغة الكود غير صحيحة",
      codeUsed: "هذا الكود مستخدم بالفعل",
      codeNotFound: "الكود غير موجود",
      accountDisabled: "تم تعطيل حسابك",
      privacy: "لا نخزن أي نتائج أو بيانات بحثك. نُسجّل عدد البحوث فقط لحماية الخدمة.",
      close: "إغلاق",
      avatarTitle: "صورة بروفايل Gravatar",
      loggedInAs: "أنت مسجل كمستخدم",
      emailNotFound: "لا توجد صورة Gravatar لهذا الإيميل",
      unknown: "غير معروف",
      searchType: "بحث",
      comprehensive: "تحليل شامل",
      comprehensiveHint: "جمع كل المعلومات ثم توليد تقرير ذكي شامل",
      analyzing: "الذكاء الاصطناعي يحلل ويبني التقرير الشامل…",
      aiTitle: "تقرير الذكاء الاصطناعي",
      aiSubtitle: "منظم من بيانات المصادر المفتوحة",
      confidence: "الثقة",
      source: "المصدر",
      riskScore: "تقييم المخاطر",
      riskLow: "منخفض",
      riskMedium: "متوسط",
      riskHigh: "مرتفع",
      exportJson: "تصدير JSON",
      exportPdf: "تصدير PDF",
      summaryLbl: "الملخص",
      personLbl: "البيانات الشخصية",
      accountsLbl: "الحسابات",
      relationsLbl: "العلاقات",
      timelineLbl: "الخط الزمني",
      notFoundField: "لا توجد بيانات",
      platformResults: "نتائج المنصات",
      techInfo: "المعلومات التقنية",
      breachesLbl: "التسريبات",
      whoisLbl: "معلومات النطاق",
      searchLinks: "روابط بحث إضافية",
      countryLbl: "الدولة",
      carrierLbl: "شركة الاتصالات",
      lineTypeLbl: "نوع الخط",
      regionLbl: "المنطقة/المدينة",
      tzLbl: "المنطقة الزمنية",
      formatsLbl: "الصيغ",
      emailLbl: "الإيميل",
      domainLbl: "النطاق",
      platformsFoundLbl: "منصات مؤكدة",
      name: "الاسم",
      age: "العمر",
      gender: "الجنس",
      location: "الموقع",
      occupation: "المهنة",
      languages: "اللغات",
      interests: "الاهتمامات",
      fullName: "الاسم الكامل",
      birthDate: "تاريخ الميلاد",
      ageEstimate: "العمر المقدر",
      cityCountry: "المدينة / الدولة",
      profession: "المهنة",
      phoneNumbers: "أرقام الهواتف",
      emails: "عناوين الإيميل",
      aliases: "أسماء مستعارة",
      extraInfo: "معلومات إضافية",
      emailProvider: "مزوّد الإيميل",
      platformsChecked: "منصات فحصت",
      platformsCheckedUnit: "منصة",
      advanced: "بحث متقدم",
      advancedHint: "بحث عميق + فحص ثغرات + ديب ويب + تقرير Qwen",
      advancedTodo: "شغّل البحث العميق أولًا؟",
      advNeedBase: "أجرِ التحليل الشامل لهذا الهدف أولًا حتى يقفز البحث المتقدم عليه",
      deepResearch: "البحث العميق",
      vulnScan: "فحص الثغرات",
      deepWeb: "مؤشرات الديب ويب",
      qwenReport: "التقرير العميق (Qwen)",
      exportCsv: "تصدير CSV",
      advTargets: "الأهداف المفحوصة",
      noVulnTargets: "لا أهداف روابط متاحة للفحص",
      severity: "الخطورة",
      sevCritical: "الحرجة",
      sevHigh: "عليا",
      sevMedium: "متوسطة",
      sevLow: "منخفضة",
      sevInfo: "معلوماتية",
      stage: "مرحلة",
      researchWayback: "الأرشيف",
      researchDorks: "استعلامات بحث",
      researchPgp: "مفاتيح PGP",
      researchShodan: "Shodan",
      researchCensys: "Censys",
      needsTor: "يتطلب TOR_PROXY",
      advSafe: "فحص سلبي فقط — لم تُنفَّذ أي هجمات",
      patternsLbl: "الأنماط",
      contradictionsLbl: "تناقضات",
      vectorsLbl: "اتجاهات بحث مقترحة",
      criticalAssetsLbl: "أصول حساسة",
      relationshipsLbl: "العلاقات",
      proAdvOnly: "التقرير العميق (Qwen) متاح فقط للحسابات البرو",
    },
    en: {
      tagline: "Discover Your Digital Footprint",
      search: "Search",
      searching: "Searching...",
      resultsFor: "Results for",
      username: "Username",
      email: "Email",
      phone: "Phone",
      placeholder_username: "e.g. darkhacker",
      placeholder_email: "e.g. name@example.com",
      placeholder_phone: "e.g. +1 555 123 4567",
      help_username: "Check a username across 3000+ sites",
      help_email: "Check an email on 120+ platforms + Gravatar image",
      help_phone: "Extract technical information about a phone number",
      quota: "Searches today",
      pro: "PRO",
      login: "Login",
      register: "Sign up",
      logout: "Logout",
      activatePro: "Activate PRO code",
      welcome: "Welcome",
      loginTitle: "Login",
      registerTitle: "Create account",
      emailLabel: "Email",
      passwordLabel: "Password",
      noAccount: "No account? Sign up now",
      hasAccount: "Have an account? Login",
      proTitle: "Activate PRO code",
      proSub: "Enter your activation code for unlimited searches for 30 days",
      codeLabel: "Code (XXXX-XXXX-XXXXX-XXX)",
      codePlaceholder: "e.g. HSDFN-DSLF-84357-FFJ",
      activate: "Activate",
      cancel: "Cancel",
      status_registered: "Confirmed",
      status_not_registered: "Not registered",
      status_rate_limited: "Rate limited",
      status_warning: "Warning",
      status_error: "Error",
      foundResults: "Results found",
      resultsLabel: "confirmed results",
      noResults: "No results found",
      done: "Done — results count",
      seconds: "seconds",
      searchFailed: "Search failed",
      limitReached: "Free daily search limit reached — activate a PRO code to continue",
      loginRequired: "Please log in first",
      welcomeBack: "Logged in successfully",
      registered: "Account created successfully",
      proActivated: "PRO activated — unlimited searches until",
      weakPassword: "Password is too short",
      exists: "This email is already registered",
      badPassword: "Incorrect password",
      notFound: "No account with this email",
      badFormat: "Invalid code format",
      codeUsed: "This code is already used",
      codeNotFound: "Code not found",
      accountDisabled: "Your account has been disabled",
      privacy: "We do not store any search results or your data. We only record search counts to protect the service.",
      close: "Close",
      avatarTitle: "Gravatar profile picture",
      loggedInAs: "You are logged in as",
      emailNotFound: "No Gravatar image for this email",
      unknown: "Unknown",
      searchType: "Search",
      comprehensive: "Deep Analysis",
      comprehensiveHint: "Collect everything, then generate a full AI report",
      analyzing: "AI is analyzing and building the full report...",
      aiTitle: "AI Report",
      aiSubtitle: "Structured from open-source intel",
      confidence: "Confidence",
      source: "Source",
      riskScore: "Risk score",
      riskLow: "Low",
      riskMedium: "Medium",
      riskHigh: "High",
      exportJson: "Export JSON",
      exportPdf: "Export PDF",
      summaryLbl: "Summary",
      personLbl: "Personal data",
      accountsLbl: "Accounts",
      relationsLbl: "Relationships",
      timelineLbl: "Timeline",
      notFoundField: "No data available",
      platformResults: "Platform results",
      techInfo: "Technical info",
      breachesLbl: "Breaches",
      whoisLbl: "Domain info",
      searchLinks: "Extra search links",
      countryLbl: "Country",
      carrierLbl: "Carrier",
      lineTypeLbl: "Line type",
      regionLbl: "Region / City",
      tzLbl: "Timezone",
      formatsLbl: "Formats",
      emailLbl: "Email",
      domainLbl: "Domain",
      platformsFoundLbl: "Confirmed platforms",
      name: "Name",
      age: "Age",
      gender: "Gender",
      location: "Location",
      occupation: "Occupation",
      languages: "Languages",
      interests: "Interests",
      fullName: "Full name",
      birthDate: "Birth date",
      ageEstimate: "Estimated age",
      cityCountry: "City / Country",
      profession: "Profession",
      phoneNumbers: "Phone numbers",
      emails: "Emails",
      aliases: "Aliases",
      extraInfo: "Extra info",
      emailProvider: "Email provider",
      platformsChecked: "Platforms scanned",
      platformsCheckedUnit: "sites",
      advanced: "Advanced",
      advancedHint: "Deep research + vuln scan + dark web + Qwen report",
      advancedTodo: "Run deep research first?",
      advNeedBase: "Run Deep Analysis for this target first so Advanced can build on it",
      deepResearch: "Deep Research",
      vulnScan: "Vulnerability scan",
      deepWeb: "Dark web indicators",
      qwenReport: "Deep report (Qwen)",
      exportCsv: "Export CSV",
      advTargets: "Scanned targets",
      noVulnTargets: "No URL targets available",
      severity: "Severity",
      sevCritical: "Critical",
      sevHigh: "High",
      sevMedium: "Medium",
      sevLow: "Low",
      sevInfo: "Info",
      stage: "Stage",
      researchWayback: "Archive",
      researchDorks: "Search dorks",
      researchPgp: "PGP keys",
      researchShodan: "Shodan",
      researchCensys: "Censys",
      needsTor: "Requires TOR_PROXY",
      advSafe: "Passive scan only - no attacks executed",
      patternsLbl: "Patterns",
      contradictionsLbl: "Contradictions",
      vectorsLbl: "Suggested next vectors",
      criticalAssetsLbl: "Critical assets",
      relationshipsLbl: "Relationships",
      proAdvOnly: "Deep Qwen report is available to PRO accounts only",
    },
  };

  const state = { lang: localStorage.getItem("lax_lang") || (navigator.language || "").startsWith("ar") ? "ar" : "en" };

  function t(key) {
    return (DICT[state.lang] && DICT[state.lang][key]) || DICT.en[key] || key;
  }

  const ATTR_KEYS = ["placeholder", "data-i18n", "data-help"];
  function applyLang() {
    document.documentElement.lang = state.lang;
    document.documentElement.dir = state.lang === "ar" ? "rtl" : "ltr";
    document.querySelectorAll("[data-i18n]").forEach((el) => {
      const k = el.getAttribute("data-i18n");
      if (el.tagName === "INPUT" || el.tagName === "TEXTAREA") el.setAttribute("placeholder", t(k));
      else el.textContent = t(k);
    });
    document.querySelectorAll("[data-help]").forEach((el) => {
      el.setAttribute("data-help-label", t(el.getAttribute("data-help")));
    });
    const btn = document.getElementById("langBtn");
    if (btn) btn.textContent = state.lang === "ar" ? "EN" : "عربي";
  }

  function setLang(lang) {
    state.lang = lang;
    localStorage.setItem("lax_lang", lang);
    applyLang();
  }

  window.laxI18n = { t, state, setLang, applyLang };
})();